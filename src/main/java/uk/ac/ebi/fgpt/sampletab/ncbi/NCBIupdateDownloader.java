...
public class NCBIupdateDownloader {
    ...
    /**
     * performs esearch to get all samples
     * that have been updated or published for a range of dates
     * and returns a map with all the results
     * the keys are db, count, query_key,
     * WebEnv and uids
     */
    public void esearch() {
        ...
        String dateRange = startyear + "/" + startmonth + "/" + currentdate + ":" + endyear + "/" + endmonth + "/"
                + rangedate;
        String term = dateRange + "[MDAT]+OR+" + dateRange + "[PDAT]+AND+public[Filter]";
        String url = base + "esearch.fcgi?db=biosample&term=" + term + "&usehistory=y&retmax=100000";
    ...
    public void efetchBatch() {
        ...
        File file;
        try {
            file = new File("/home/drashtti/Desktop/BioSamples/ncbi_uploader.xml");
        ...
        if (results.get("query_key") != null && results.get("WebEnv") != null) {
            url = url + "&querykey=" + results.get("query_key") + "&WebEnv=" + results.get("WebEnv")
                    + "&retmode=xml&rettype=full";
        } else {
            System.out.println("WARNING: eFetch requires Query Key and WebEnv !!");
        }

        int retmax = 500;
        int last = 0;
        BufferedReader in = null;
        Integer count = Integer.parseInt(results.get("count")); // the size of the dataset
        for (int retstart = 0; retstart < count; retstart = retmax + retstart) {
            try {
                ...
                try {
                    ...
                    System.out.println("WARNING: Could not open HTTP connection");
                    e.printStackTrace();
                }
    ...
    public void efetch_Id() {
        int count = Integer.parseInt(results.get("count")); // the size of the dataset
        int retmax = Integer.parseInt(results.get("retMax")); // no. of ids retrieved
        ...
        if (count > retmax) {
            ...
            System.err.println("The number of Biosample IDs retrieved does not correspond");
        } else {
            String url = base + "efetch.fcgi?db=biosample";
            for (String uid : uids) {
                ...
                try {
                    ...
                    System.out.println("WARNING: CANNOT COMPLETE THE CONNECTION");
                    e.printStackTrace();
                }
	...
	public void efetchBatch(){
		...
		if (results.get("query_key") != null && results.get("WebEnv") != null){
			url = url+"&querykey="+results.get("query_key")+"&WebEnv="+results.get("WebEnv")+"&retmode=xml&rettype=full";
				}
		else{
			System.out.println("WARNING: eFetch requires Query Key and WebEnv !!");
		}
	...
	public void efetch_Id(){
		...
		int uid_list = uids.size(); //no. of ids in the collected set
		if (count > retmax){
			System.err.println("The number of entries modified/published in last week is greater than those retrieved i.e. 100,000");
		}
		else if (count != uid_list){
			System.err.println("The number of Biosample IDs retrieved does not correspond");
		}
